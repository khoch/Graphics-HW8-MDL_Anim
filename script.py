...
def first_pass( commands ):
    ...
    for command in commands:
        if command[0] == "frames":
            if frames:
                sys.exit("Error: 'frames' called more than once.")
            else:
                frames = command [1]
        elif command[0] == "frames":
            if basename:
                sys.exit("Error: 'basename' called more than once.")
            else:
                basename = command [1]
        elif command[0] == "vary":
    if not frames:
    ...
    if not basename: 
        basename = "default"
        print "setting basename as 'default'"
    if frames and not vary:
        sys.exit("Error: 'vary' called but not 'frames'.")
